...
for i in range(0, len(t)):
    ...
    if night_id[i] == 0:
        new_m.append(m[i]-0.023)
    elif night_id[i] == 1:
        new_m.append(m[i]-0.15)
    elif night_id[i] == 2:
...
ax1.text(0.773, mean_val + 0.0125, f'Mean: {mean_val:.5f} $\\pm$ {mean_err:.5f}', 
...
amp_text = f'Amplitude: {amp_pp_best:.3f} $\\pm$ {amp_pp_err:.3f} mag' 
...
left = 0.745    # Distance from left edge
...
plt.tight_layout()
plt.savefig("Figures/rcurve_with_residuals.png", dpi=300, bbox_inches='tight')
plt.show()